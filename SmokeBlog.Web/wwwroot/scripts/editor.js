(function (global) {
     ...
     */
    function createIcon(name, icon, options) {
        options = options || {};
        ...
        el.className = options.className || 'fa fa-' + icon;
    ...
    var toolbar = [
      ...
      { name: 'quote', icon: 'quote-left', action: toggleBlockquote },
      { name: 'unordered-list', icon: 'list-ul', action: toggleUnOrderedList },
      { name: 'ordered-list', icon: 'list-ol', action: toggleOrderedList },
      '|',

      { name: 'link', icon: 'link', action: drawLink },
      { name: 'image', icon: 'image', action: drawImage },
      '|',

      { name: 'info', icon: 'info', action: 'http://lab.lepture.com/editor/markdown' },
      { name: 'preview', icon: 'eye', action: togglePreview },
      { name: 'fullscreen', icon: 'arrows-alt', action: toggleFullScreen }
    ];
    ...
    Editor.prototype.createToolbar = function (items) {
        ...
        for (var i = 0; i < items.length; i++) {
            (function (item) {
                var el;
                if (item.name) {
                    el = createIcon(item.name, item.icon, item);
                } else if (item === '|') {
                    el = createSep();
                } else {
                    el = createIcon(item);
