...
angular.module('blogAdmin').config(['$routeProvider', '$locationProvider', function ($routeProvider, $locationProvider) {
    $routeProvider.when('/', { templateUrl: '/apps/templates/dashboard.html', controller: 'mainCtrl' }).when('/userlist', { templateUrl: '/apps/templates/userlist.html', controller: 'userListCtrl' }).when('/profile', { templateUrl: '/apps/templates/my.html', controller: 'myCtrl' }).otherwise({ redirectTo: '/' });
